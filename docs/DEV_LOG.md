...
**Session 35 — Stripe LIVE-TESTED (test mode) + fix batch + PLAN LIMITS ENFORCED:** payment flow verified by the user end-to-end (card 4242). Root cause of "paid but still Basic": **`profiles` table was empty** → backfilled + billing writes now UPSERT + `reconcileFromStripe` self-heal on the billing page (also prevents double-charging). Landing "Go Premium" → direct Checkout (`?checkout=1`); billing page = current-plan card on top + other plan below; landing navbar session-aware (email + Dashboard + Log off). **Pricing finalized & ENFORCED:** Basic = 2 triage/month (never cuts an in-flight session; follow-ups free; upgrade screen on the assessment page) · 10 assistant msgs/day (user-tz Redis counter, `/api/assistant`) · 1 pet (POST) · exports unlimited for all. `src/lib/plan-limits.ts` = single source; 403 `code:"plan_limit"`; clients show upgrade UI (assessment chat suppresses the emergency fallback for it). Tests **156**. **User-verified:** payment flow end-to-end AND the assistant 10-message toast on a Basic account. **COMMITTED + PUSHED this session** (Sessions 34–35 in one commit). `STRIPE_SECRET_KEY` added to Vercel by the user — **prod also needs `SUPABASE_SERVICE_ROLE_KEY`** (billing writes) or paid upgrades won't persist; `STRIPE_WEBHOOK_SECRET` + dashboard webhook endpoint = pending (success-redirect confirmation covers upgrades meanwhile; cancel/renewal sync needs the webhook). Duplicate old test sub pending user cancel in Stripe dashboard. See Session 35 entry.
...
### NEXT SESSION MUST START WITH
*(End-of-session update: limits verified by the user on a Basic account — the 10-message assistant toast works; everything COMMITTED + PUSHED (`a3b30aa`). `STRIPE_SECRET_KEY` AND `SUPABASE_SERVICE_ROLE_KEY` confirmed in Vercel. Stripe cleanup done by the user: both old subs canceled + refunded, ONE fresh active sub remains and the DB profile matches it exactly (verified). **CUSTOM DOMAIN IS LIVE: `pitsypet.andreshenao.com.au` (+www) alongside pitsypet.vercel.app** — triggers the "very-end domain swap" checklist NOW: (a) Vercel `NEXT_PUBLIC_APP_URL` → new domain + redeploy (Stripe return URLs build from it — cross-domain redirect otherwise lands logged-out); (b) Stripe webhook endpoint on the new domain + `STRIPE_WEBHOOK_SECRET` in Vercel; (c) Supabase Auth Site URL + redirect allowlist add new domain; (d) UptimeRobot monitor URL. Steps given to the user this session.)*
1. User cancels the duplicate old subscription in the Stripe dashboard (test mode → Subscriptions → cancel `sub_1TuRn1…`).
2. **Prod env still missing: `SUPABASE_SERVICE_ROLE_KEY` in Vercel** (without it a prod payment succeeds at Stripe but the upgrade cannot be written — confirm/reconcile/webhook all need it). Then a prod payment test with card 4242.
3. `STRIPE_WEBHOOK_SECRET` + Stripe dashboard webhook endpoint (`https://pitsypet.vercel.app/api/billing/webhook`, events: checkout.session.completed, customer.subscription.updated, customer.subscription.deleted) — needed for cancel/renewal sync in prod.
4. Remaining Basic-limit manual checks (optional): 2-triage cap upgrade screen; 2nd-pet 403.

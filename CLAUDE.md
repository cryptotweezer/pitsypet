...
| 4 | **RAG Knowledge Base Ingestion** (TypeScript `scripts/ingest.ts`) | 🟠 **Blocked/deferred:** pipeline built; ingestion requires vetted, reusable veterinary source documents. The KB is still empty, so veterinarian-led calibration and RAG in the assistant remain pending. |
...
- **Current:** Phase 8 — UI/UX Polish & Accessibility. The rapid landing pass now includes responsive spacing work, the final footer attribution/disclaimer, branded `/privacy` and `/terms` pages, and Dashboard > Account deletion. The user owns visual checks during this pass; do not run visual automation unless explicitly requested.
...
- **Blocked/deferred:** Phase 4 source collection/ingestion and veterinarian-led calibration; RAG in the assistant until the KB exists; Resend custom auth and doctor-email flows until the required account/domain work is available.

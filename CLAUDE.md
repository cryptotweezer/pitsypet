...
| 7.5 | **Pet Clinical History Hub** (user-directed expansion: pet page + meds/vet + contextual chat + export) | ⏳ **CURRENT — Part 1 ✅; Part 2 in progress (groups A→B→C→D). A,B,C ✅ done+tested (uncommitted): orphan-assessment fix, clinic+doctors+appointments+hours, AI clinical context + follow-ups. Next: fix "new assessment merged" bug → backlog (active-symptoms tracker, appointment outcome notes, etc.) → Group D chats. Email/Resend deferred to after D.** |
...
- **Deferred items currently outstanding:** **Email/Resend (user has NO account or domain yet — set up at the very end).** This covers BOTH the deferred custom-SMTP auth email (Phase 1 task 1.5) AND the new Phase 7.5 "request appointment → email the doctor" feature (manual button + AI-sent email with the last-assessment summary) — do this as one dedicated step AFTER Group D, since the valuable version needs D's chat tools + C's summary. Also still outstanding: expired-token session refresh test (Phase 2, verified at Phase 11); mid-chat reload *recovery* (the orphan *create* path is now fixed in 7.5 Group A — rows exist only after completion). Don't lose these — all tracked in the DEV_LOG.
...
- **Done:** Phase 0–3; Phase 5 (AI triage engine); Phase 6 (results + recommendations, auto-save); Phase 7 (history+search, later folded into per-pet). **Phase 4 pipeline built but ingestion run pending user PDFs** (RAG runs empty meanwhile; classification works on model knowledge). **Current:** Phase 7.5 — **Pet Clinical History Hub** (user-directed; see Phase 7.5 in `dev_plan.md`). Built in **parts/groups**, docs updated per group. **Part 1 ✅ committed.** **Part 2 in progress — groups A→B→C→D; A, B, C ✅ done + live-tested (uncommitted):**
  - **A:** orphan-assessment fix — **a row is written only when the assessment COMPLETES** (chat route `onFinish` upserts on `complete`; the page mints a UUID, no pre-insert). Chat box fixed-height + scroll; symptom sidebar sticky.
  - **B:** **vet model is now clinic + doctors** — `vet_contacts` = clinic (`service_hours` JSONB picker + `address`), new `vet_doctors` (many per clinic), new `appointments` table. Meds editable + labeled; clinic editable; clickable Hours dialog w/ live Open/Closed; collapsible doctors. Pet page order: Vet → Appointments → Medications.
  - **C:** AI gets **current meds + last 3 assessments** as clinical context (extraction + classify); results show detected-symptom badges; history cards show an abstract; **follow-ups** = dated sections appended to the same assessment (`follow_ups` JSONB) via `?followup=<id>` + a `+ Follow-up` button + results timeline.
- **Immediate next:** fix the reported **"new assessment merged into existing"** bug (a NEW assessment must be its own row; only `+ Follow-up` appends), then the **PENDING BACKLOG** in DEV_LOG Session 12 (active-symptoms tracker, appointment outcome/past notes, prescribed-by dropdown, med date validation, sidebar context), then **Group D** (per-pet + dashboard chats w/ write tools), then **Email/Resend** (deferred — user has no account/domain yet) and **Part 3** (export + AI summary).

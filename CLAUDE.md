...
- **Immediate next (RESUME HERE): run the TESTING PASS** in `docs/TESTING_PROTOCOL.md` (Groups 0–I, incl. PDF export) and fix anything in its "Failures to fix" section. Only after the test pass: **Email/Resend** (Group E — user has no account/domain yet) and **RAG into the assistant chat** (KB empty until Phase 4 ingestion). See **Triage calibration & tuning** above for the over-escalation work (vet-driven, with Phase 4 RAG).

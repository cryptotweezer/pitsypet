...
| 4 | **RAG Knowledge Base Ingestion** (TypeScript `scripts/ingest.ts`) | 🟠 **Blocked/deferred:** pipeline built; ingestion requires vetted, reusable veterinary source documents. The KB is still empty and RAG in the assistant stays pending. **No longer blocks calibration:** the veterinarian's thresholds were captured directly in Session 42 (`docs/vet_calibration_notes.md`); RAG will later reinforce them, not replace them. |
...
- **Blocked/deferred:** Phase 4 source collection/ingestion (the vet did not supply source documents, so this stayed blocked after Session 42); RAG in the assistant until the KB exists; real emergency contacts, still unverified; Australia-specific exposures, not covered by the Colombian vet; Resend custom auth and doctor-email flows until the required account/domain work is available.

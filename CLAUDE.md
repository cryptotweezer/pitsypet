...
- **Immediate next (RESUME HERE): run the TESTING PASS** in `docs/TESTING_PROTOCOL.md` (full Groups 0–H checklist) and fix anything in its "Failures to fix" section. Only after the test pass: **Email/Resend** (Group E — user has no account/domain yet), **Part 3** (export + AI summary), **RAG into the assistant chat** (KB empty until Phase 4 ingestion).

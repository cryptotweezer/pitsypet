...
| 11 | Monitoring & Production Deployment (Sentry, PostHog, UptimeRobot health check) | 🟢 **Done bar manual smoke:** Sentry (11.1), PostHog + 3 named events (11.2), `/api/health` + **UptimeRobot 5-min monitor "Up"** (11.3), **Supabase Auth URLs set** (11.4); AI keys in Vercel confirmed (prod AI works). **Left:** 11.6 prod smoke test (manual). **CUSTOM DOMAIN LIVE (Session 35): prod = `https://pitsypet.andreshenao.com.au`** (+www; pitsypet.vercel.app still attached) — the full swap checklist is DONE: Vercel `NEXT_PUBLIC_APP_URL`, Stripe webhook endpoint on the new domain, Supabase Site URL + redirect allowlist (new domain + www + vercel.app + localhost), UptimeRobot monitor URL. |
...
- **Plan tiers — Basic vs Premium. STRIPE BILLING IS BUILT (Session 34, test mode) AND PLAN LIMITS ARE ENFORCED (Session 35).** Limits (PitsyBasic; spec + copy agreed with the owner): **2 AI triage sessions/month** (gates STARTING a new assessment only — a session under way or a follow-up is NEVER cut off mid-conversation; enforced in the assessment chat route + a friendly upgrade screen on the assessment page), **10 AI assistant messages/day** (assistant chats only — dashboard widget + pet panel; Redis counter keyed to the user's local day; enforced in `/api/assistant`), **1 pet profile** (pets POST; existing extra pets untouched), **vet PDF exports UNLIMITED on every plan** (deterministic, no AI cost). All limits live in `src/lib/plan-limits.ts` (`BASIC_LIMITS` + `LIMIT_MESSAGES` — landing/billing copy mirrors these numbers, change together). Limit responses are `403 { error, code: "plan_limit" }`; clients parse via `parseApiError` (`src/lib/utils.ts`) — the assessment chat shows an upgrade notice (NOT the emergency fallback), the assistant shows a toast with a Go Premium action. PitsyPremium $9.99/mo AUD via **Stripe Checkout** (hosted) + **Customer Portal** (payment history, invoice PDFs, card update, cancel — all Stripe-hosted, no custom invoice code). `profiles.plan` (`basic`/`premium`) is the source of truth, maintained ONLY server-side on Stripe-verified evidence: `POST /api/billing/webhook` (signature-checked; public in middleware) and the success-redirect confirmation in `/dashboard/billing` (works locally without the Stripe CLI). Users CANNOT self-upgrade: column-level `REVOKE UPDATE` on the billing columns (migration `20260718000000` — any future user-editable profile column must be added to `GRANT UPDATE (name, state, …)`). Price auto-creates by `lookup_key pitsypet_premium_monthly` — going live = swap `sk_live_` keys, zero code/dashboard setup. The landing pricing section reflects the tier limits. **PitsyBasic (free):** 2 AI triage sessions / month · 2 vet PDF exports / month · **1 pet profile** · Low/Medium/High risk assessment · 24/7 access, any device. **PitsyPremium:** everything **unlimited** — unlimited AI triage sessions, unlimited vet PDF exports, **unlimited pets**, **unlimited records & clinical history**, priority support. **Enforcement points when built:** assessment-create route (monthly triage count), export route (monthly export count — `POST /api/assessment/[id]/export` already has `exportRateLimiter` + a daily cap to build on), pet-create route (1-pet cap for Basic). Stripe checkout is still stubbed — the "Go Premium"/"Start free" buttons point at `/register` (see the TODO comment in `src/app/page.tsx`).

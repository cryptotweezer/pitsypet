...
| 7.5 | **Pet Clinical History Hub** (user-directed expansion: pet page + meds/vet + contextual chat + export) | ⏳ **CURRENT — feature work COMPLETE, in TESTING. Parts 1 + 2 (A–D), Session 14 (global vet clinics + tz dates + assessment prompt fixes), Session 15 (collapsible past appts/finished meds + "mark finished"; assessment-complete = in-chat "View results" link + chat locks, no auto-redirect; appointment `doctor_name` by-clinic datalist; results page shows emergency contacts for High follow-ups; pet-card tag tracks latest follow-up; vet clinic/doctor hard-delete), and Session 16 (**Part 3 — vet PDF export + AI summary**: `@react-pdf/renderer`, `POST /api/assessment/[id]/export` assembles record + Sonnet summary, current/past meds with dates, per-block risk recommendations) all ✅ committed. **NEXT (resume here): run the testing pass in `docs/TESTING_PROTOCOL.md` (now incl. Group I — PDF export), fix any failures.** Then Email/Resend (Group E) + RAG-in-chat. Export-summary caching is a noted deferred optimization.** |
...
| 11 | Monitoring & Production Deployment (Sentry, PostHog, UptimeRobot health check) | 🟢 **Monitoring done:** Sentry (11.1), PostHog + 3 named events (11.2), `/api/health` (11.3) with **UptimeRobot HTTP monitor live (5-min, "Up")**. Env vars set in Vercel. **Left:** 11.4 confirm Supabase Auth redirect URLs = prod domain; 11.6 prod smoke test (manual). NOTE: prod URL is `pitsypet.vercel.app` — will move to a custom domain at the very end (update UptimeRobot + Supabase redirect URLs then). |
...
- **Deferred items currently outstanding:** **Email/Resend (user has NO account or domain yet — set up at the very end).** This covers BOTH the deferred custom-SMTP auth email (Phase 1 task 1.5) AND the new Phase 7.5 "request appointment → email the doctor" feature (manual button + AI-sent email with the last-assessment summary) — do this as one dedicated step AFTER Group D, since the valuable version needs D's chat tools + C's summary. Also still outstanding: expired-token session refresh test (Phase 2, verified at Phase 11); mid-chat reload *recovery* (the orphan *create* path is now fixed in 7.5 Group A — rows exist only after completion); **RAG wiring into the Group D assistant chat (deferred until Phase 4 KB ingestion — no content to retrieve yet).** Don't lose these — all tracked in the DEV_LOG.
- **Vet PDF export — AI summary is NOT cached (deferred optimization):** the Part 3 export (`POST /api/assessment/[id]/export`) calls Sonnet to write the vet summary on **every** download, so re-printing the same assessment = repeated API calls + cost + latency. Fix when worthwhile: store the generated summary on the assessment (e.g. a `vet_summary` JSONB + `vet_summary_generated_at` column), reuse it by default, and add a **"Regenerate summary"** action (or auto-regenerate only when the inputs changed — a new follow-up or a medication change). Deterministic data (patient, meds, blocks) is assembled fresh either way; only the AI prose needs caching.
...
- **Immediate next (RESUME HERE):** (a) **decide the Next.js advisories** (stay on 14.2.35 with residuals vs schedule a Next 15 upgrade); (b) independent track — **Phase 10 Vitest** triage regression set and/or the **TESTING PASS** in `docs/TESTING_PROTOCOL.md` (Groups 0–I, incl. PDF export). Deferred: **Email/Resend** (Group E — no account yet) and **RAG into the assistant chat** (KB empty until Phase 4). See **Triage calibration & tuning** above for the over-escalation work (vet-driven, with Phase 4 RAG).
